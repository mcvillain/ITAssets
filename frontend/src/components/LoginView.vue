  ...
  methods: {
    async login() {
      if (this.input.username.length > 0 && this.input.password.length > 0) {
        const loginReq = await axios.get(import.meta.env.VITE_API_ENDPOINT+'/login', {
          username: this.input.username,
          password: this.input.password,
        });
        if (loginReq.status == 200) { //Success
          VueCookies.set('session_id', loginReq.data, 28800); // Valid for 8 hours
          const auth_lvl = await axios.get(import.meta.env.VITE_API_ENDPOINT+'/auth');
          if (auth_lvl.status == 200 && auth_lvl.data == "2") {
            location.href='/admin';
          }
          location.href='/';
        } else if (loginReq.status == 401) { // Unauthorized
          VueCookies.remove('session_id');
          this.output = "Login failed. Check your username and password.";
        }
      }
      // try {
