...
export default {
    ...
    async setup() {
        if (location.pathname !== '/') {
            this.username = await fetch(import.meta.env.VITE_API_ENDPOINT + "/auth", { credentials: 'include' }).then((resp)=>{if (resp.status === 401) {location.href = '/';}return resp.json()}).then((body) => {let auth_lvl = body.auth_lvl;if(!(auth_lvl == 1 || auth_lvl == 2 || auth_lvl == 3)){location.href='/';}return body.username;});
        }
...
<template>
    <div v-if="$router.currentRoute.value.fullPath !== '/'" id="username_float">Logged in as: {{ username_ref }}</div>
    <div class="sidebar" :style="{ width: sidebarWidth }">
        ...
        <div class="item" :class="collapsed && 'item collapsed'" style="display: flex; flex-direction: row;">
            <a href="/api/logout_redirect">
                <i class="bi bi-box-arrow-left help-icon"></i></a>
            <span v-if="!collapsed">
                <a href="/api/logout_redirect">
