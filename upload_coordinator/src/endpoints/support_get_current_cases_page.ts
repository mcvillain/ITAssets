...
export async function get_support_get_current_cases_pages(req: Request, res: Response ) {
       ...
       const response = await execute_sql (`SELECT * FROM cases WHERE owner = '${owner}' ORDER BY created_at DESC LIMIT ${ITEMS_PER_PAGE} OFFSET ${skipped}`);
