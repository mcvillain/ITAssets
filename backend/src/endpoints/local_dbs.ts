...
function update_dbs_sql(current_dbs: LocalDatabase[]) {
    current_dbs.forEach((db: LocalDatabase) => {
        const sql_query = `INSERT INTO local_dbs (dbkey, name, size, paths, created, database_id, LastCheckInTime${db.version===undefined||db.version==='null'?'':', version'}) VALUES ('${db.name}_${db.database_id}', '${db.name}', '${db.size}', '${db.paths.join('|')}', '${db.created}', ${db.database_id}, '${new Date().toUTCString()}'${db.version===undefined?'':', \''+db.version+'\''})`;
